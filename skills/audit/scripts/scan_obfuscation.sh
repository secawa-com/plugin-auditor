...
DECODE_RE = re.compile(
    ...
    r"String\.fromCharCode|bytes\s*\(\s*\[",
    re.IGNORECASE,
)
# Any execution primitive across common languages.
EXEC_RE = re.compile(
    r"\bexec\s*\(|\beval\s*\(|\bcompile\s*\(|runpy|"
...
for root, dirs, files in os.walk(repo):
    ...
    for fn in files:
        ...
        # Cross-line: a decode anywhere in the file followed later (or nearby) by
        # an execution primitive. Report the exec site; note the decode line.
        if decode_lines and exec_lines:
            for el in exec_lines:
                # nearest preceding decode
                prior = [d for d in decode_lines if d <= el]
                near = [d for d in decode_lines if abs(d - el) <= 25]
                if prior or near:
                    src = (prior or near)[-1] if prior else near[0]
                    if src != el:  # same-line already reported above
